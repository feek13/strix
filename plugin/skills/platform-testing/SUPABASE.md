...
### Deep Verification for Write Operations

For PATCH/DELETE operations, always verify actual data changes:
- Use `Prefer: return=representation` header - empty array `[]` = RLS blocked = no vulnerability
- Compare before/after state to confirm actual modification
- See [Deep Verification Methodology](../verification-methods/DEEP_VERIFICATION.md) for detailed patterns
