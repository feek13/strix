...
- Pages intentionally public with documented caching behavior
- Proper Vary headers and cache keys including Authorization/tenant
- Server actions with explicit authorization checks independent of client state
- Middleware consistently applied across all runtimes

## Impact

- Cross-account/tenant data exposure via cache poisoning
- Unauthorized access to admin/preview content
- Session hijacking via NextAuth misconfigurations
- SSRF via image optimizer to internal services
